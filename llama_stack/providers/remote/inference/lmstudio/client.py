    ...
    async def llm_respond(
        ...
        if tools is None or len(tools) == 0:
            ...
            if stream:
                ...
                response = await asyncio.to_thread(llm.respond, history=chat, config=config, response_format=json_schema)
    ...
    async def llm_completion(
        ...
        if stream:
            async def stream_generator():
                ...
                prediction_stream = await asyncio.to_thread(
                    llm.complete_stream, prompt=interleaved_content_as_str(content), config=config, response_format=json_schema
                )
