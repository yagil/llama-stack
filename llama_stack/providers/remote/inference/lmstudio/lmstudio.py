...
class LMStudioInferenceAdapter(Inference, ModelsProtocolPrivate):
    def __init__(self, config: LMStudioImplConfig) -> None:
    ...
    async def initialize(self) -> None:
        log.info(f"Initializing LMStudio client with base_url={self.config.url}")
    ...
    async def register_model(self, model: Model) -> Model:
        ...
        if model.provider_resource_id not in available_models:
            raise ValueError(
                f"Model {model.provider_resource_id} is not being served by LM Studio. "
    ...
    async def _get_params(self, request: Union[ChatCompletionRequest, CompletionRequest]) -> dict:
        ...
        if isinstance(request, ChatCompletionRequest):
            ...
            assert not request_has_media(request), "LM Studio does not support media for Completion requests"
        ...
        if fmt := request.response_format:
            if fmt.type == ResponseFormatType.json_schema.value:
                input_dict["extra_body"] = {"response_format": { "type": "json_schema", "json_schema": request.response_format.json_schema} }
    ...
    async def embeddings(
        ...
        assert all(not content_has_media(content) for content in contents), "LMStudio does not support media for embeddings"
