...
from llama_stack.apis.inference import LogProbConfig, JsonSchemaResponseFormat
...
def provider_tool_format(inference_provider_type):
    return PROVIDER_TOOL_PROMPT_FORMAT[inference_provider_type]
...
def test_completion_structured_output(llama_stack_client, text_model_id):
    user_input = """
    ...
    response = llama_stack_client.inference.completion(
        ...
        response_format=JsonSchemaResponseFormat(
            json_schema=StructuredOutput.model_json_schema(),
        ),
    )
    print(f"!!! {response.content} !!!")
    answer = StructuredOutput.model_validate_json(response.content)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("How many days in a year (as a number)?", "365"),
    ],
)
def test_text_chat_completion_streaming(
    ...
    assert expected.lower() in streamed_content


def test_text_chat_completion_with_tool_calling_and_no_streaming(
...
def test_chat_completion_structured_output(llama_stack_client, text_model_id):
    pass
